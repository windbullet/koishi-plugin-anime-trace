...
export function apply(ctx: Context, config: Config) {
  ctx.command("以图识人.动画 [img:image]", "识别图片中的动画角色")
    ...
    .action(async ({session, options}, img) => {
      let model = options.model === "1" ? "anime_model_lovelive" : "anime_model"
      return h.quote(session.messageId) + await detect(model, img, session)
    })

  ctx.command("以图识人.gal [img:image]", "识别图片中的galgame角色")
    .action(async ({session}, img) => {
      return h.quote(session.messageId) + await detect("game_model_kirakira", img, session)
    })
